...
import os
from creation_csv import telechargement

def all_infos_livre(url, chemin_images) :
    """
    Fonction qui prend en paramètre un lien url.
    Elle va scrap des données via le lien.
    Elle appelle la fonction de téléchargement qui télécharge et enregistre les images dans un dossier dédié.
    """
    ...
    for next_page in new_link :
        ...
        for i in links :
            ...
            #Appel de la fonction telechargement, qui va telecharger et renomer les images
            #Elle renvoi egalement le lien vers le dossier ou sont les images
            chemin_image = telechargement(titre_livre, image_couverture, chemin_images)

            desc_livre = [link_book, info_livre[0],titre_livre, info_livre[3], info_livre[2], info_livre[5], description_livre, catagory_livre, info_livre[6], image_couverture, chemin_image]
